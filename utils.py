...
class Topic:
    ...
    # def __reduce__(self):
    #     # __reduce__ should return a tuple with a callable (constructor) and arguments to be passed to it
    #     # In this case, we're returning the class constructor and the arguments needed to recreate the instance
    #     return (self.__class__, (self.name,))
    
    def __getstate__(self):
        # Define what should be serialized when pickling an instance
        state = self.__dict__.copy()  # Save the current state of the object
        return state

    def __setstate__(self, state):
        # Define how to restore the object's state when unpickling
        self.__dict__.update(state)  # Update the object's state with the pickled state

    
    def generate_problems(self, num_problems=5, num_classes=5):
        template = """we are a company that build {main_topic}, so far we have the problems: {current_problems}. 
        give me a list of {num} additioal problems about our product that we need to solve. explaine each problem in one sentance.
        ...
        prompt = PromptTemplate(input_variables={"main_topic", "num", "current_problems"}, template=template)
        chain = LLMChain(llm=llm, prompt=prompt)
        problems = chain.run(main_topic = self.name, num = num_problems, current_problems=self.get_problems_descriptions())
        print(problems)
    ...
    def combain_simillar_problems(self, problems):
        template = """you are an AI assistent, I have sentences that describes problems with {topic}. 
        please help me arrange those sentences so if there are some sentences that discibe the 
        same problem or that has similar meaning merge them into 1 problem.
sentenses: {problems}
"""#output the merged problems as python list"""
        llm = ChatOpenAI(temperature = 0, model="gpt-4")
        prompt = PromptTemplate(input_variables={"topic", "problems"}, template=template)
        chain = LLMChain(llm=llm, prompt=prompt)
        problems = chain.run(topic = self.name, problems = problems)
        return problems
    ...
    def classify_problems(self, problems_str, to_merge=True, num_classes=5):
        self.problems = []
        if to_merge:
            print("go to merge")
            problems_str = self.combain_simillar_problems(problems_str)
            print("after combine: \n", problems_str)
        ...
        problems: {problems}.
        output in the format of python list of dictionary: [["main_class": "main_class", "problem_name": "problem_name", "description": "description", "reporter": "reporter"],...]"""
        ...
        for prob in prob_list:
            print(prob)
            self.problems.append(Problem(self.name, prob["description"], prob["main_class"], prob["problem_name"], prob["reporter"]))
    ...
    def add_problems(self, problems):
        ...
        template = """given the classifictions: {problems_classes} and problems: {descriptions}, and a new problems: {problems}.
    if the new problem is already exist in the problem list (could be in defferent words) return only the word "exist" without any introduction to your result. 
        ...
        new class:
    """
        prompt = PromptTemplate(input_variables={"problems_classes", "descriptions", "problems"}, template=template)
        chain = LLMChain(llm=llm, prompt=prompt)
        result = chain.run(problems_classes = self.get_problems_classes(), descriptions = self.get_problems_descriptions() ,problems = problems)
        print(result)
        ...
        self.problems.append(Problem(self.name, prob["description"], prob["class"], prob["problem_name"], prob['reporter']))
    ...
    def plot_hierarchy_solutions(self):
        G = nx.Graph()
        G.add_node(self.name)
        for s in self.solutions:
            print(s.sub_class, s.main_class)
            G.add_node(s.main_class)
            G.add_node(s.sub_class)
            G.add_edge(self.name, s.main_class)
            G.add_edge(s.main_class, s.sub_class)

        fig, ax = plt.subplots(figsize=(20, 10)) #figure
        layout = nx.drawing.nx_agraph.graphviz_layout(G, prog='dot')
        nx.draw(G, pos=layout, with_labels=True, font_size=10, font_weight='bold', node_size=1000)#, node_color=node_colors)
        plt.show()
        return fig
    
    def combain_simillar_solutions(self, solutions):
        template = """you are an AI assistent, I have sentences that describes solutions with {topic}. pls help me arrange those sentences so if there are some sentences that discibe the 
        same solution or that has similar meaning merge them into 1 solution.
sentenses: {solutions}"""
        llm = ChatOpenAI(temperature = 0, model="gpt-3.5-turbo")
        prompt = PromptTemplate(input_variables={"topic", "solutions"}, template=template)
        chain = LLMChain(llm=llm, prompt=prompt)
        solutions = chain.run(topic = self.name, solutions = solutions)
        return solutions
    
    # given string of list of problems, classify them to main and sub classes, and create problem objects and enter to the topic list
    def classify_solutions(self, solution_str, to_merge=True, num_classes=5):
        self.solutions = []
        print("start classify solutions")
        kpi_list = ['cost', 'feasibility', 'efficiency', 'Scalability', 'Reliability']
        if to_merge:
            solution_str = self.combain_simillar_solutions(solution_str)
        llm = ChatOpenAI(temperature = 0.5, model="gpt-4")
        ...
        evaluate and grade each solution from 1 to 5 on the following kpi: {kpi}
        solutions: {solutions}. 
        output in the format of python list of dictionary: ["main_class": "main_class", "sub_class": "sub_class", "description": "description1", "grade": grade_dict...]
        grade_dict: ["cost": , "feasibility": , "efficiency": , "Scalability": , "Reliability": ]
        """
        

        prompt = PromptTemplate(input_variables={"solutions", "kpi"}, template=classify_template)
        chain = LLMChain(llm=llm, prompt=prompt)
        result = chain.run(solutions = solution_str, kpi = kpi_list)#, num_classes = num_classes)
        sols_list = ast.literal_eval(result)
    ...
    def build_knowledge_graph(self):
        ...
        fig = drow_KG(self.kg, edges_labels)
        return fig
        
    #generate chllenge from the problem list with the given index
    def create_challenge(self, problem_indexes = [0]):
        if len(self.problems) == 0:
            print("there is no problems in this topic")
            return
        if max(problem_indexes) >= len(self.problems):
            print("problem index is out of range")
            problem_indexes = [0]
            
        problems = [self.problems[prob_index] for prob_index in problem_indexes]
        template = """ pls write a simple challenge for the innovation team that improves {main_topic} {problems}
        ...
        prompt = PromptTemplate(input_variables={"main_topic", "problems"}, template=template)
        llm = ChatOpenAI(temperature = 0.3, model='gpt-3.5-turbo')
        chain = LLMChain(llm=llm, prompt=prompt)
        problems_str = ""
        for prob in problems:
            problems_str += prob.description + ", "
        result = chain.run(main_topic=problems[0].main_topic, problems=problems_str)
        kpi = ast.literal_eval(result[result.find("["):result.find("]")+1])
        result = result[:result.find("Python list")-1]  
        self.challenges.append(Challenge(self.name, problems[0].sub_class, result, kpi))
...
"""
        ...
        self.triplets = triplets_str.split("\n")
        print("triplets: ", self.triplets)
        
        for factor in self.factors:
            # print(factor + " -[cause]-> " + self.sub_class)
            self.triplets.append(factor.name + " -[cause]-> " + self.sub_class)
        # print(self.triplets)
    ...
    def build_knowledge_graph(self):
        ...
        fig = drow_KG(self.kg, self.edges_labels)
        return fig
    ...
    {sub_class} : {problem}""".format(main_class=self.main_class, sub_class=self.sub_class, problem=self.description)
        return prob_str
...
class Challenge:
    ...
    def classify_solutions(self, solutions_str):
        ...
        classify_template = """classify the below solution into hierarchical classes give each class short and meaningful name. the main class should indecate
        what the solution is fixing, and the sub class should indecate how the solution is fixing it. both classes names should be short - no more than 3 words.
        add the solution themself. don't add any introduction to your result.
        solutions: {solutions}. 
    ...
    def update_sulotions(self, new_data):
        for sol in self.solutions:
            sol.update_solution(new_data)
...
def plot_solutions_polygons(solutions, to_show = True):
    max_score = 5
    ...
    colors = ["b", "g", "k", "y"]  # Define colors for each solution
    colors_dict = {'b':'blue', 'g':'green', 'p':'purple', 'o':'orange', 'k':'black', 'y':'yellow'}
...
class Solution:
    def __init__(self, description, main_class, sub_class, grades, user = None):
        self.description = description
        self.summary = description  #general_summary(description)# temp = 0.3, max_words = 1
        self.user = user
    ...
    def update_solution(self, free_text):
        os.environ["OPENAI_API_KEY"] = OPENAI_API_KEY
        update_template = """" given the solution: {solution}, pls update the solution with data from the following text: {text}.
...
def save_chat_history(messages, top):
    summ_messages = general_summary(messages, temp=0, max_sentances = 10)
    top.challenges[0].update_sulotions(summ_messages)
